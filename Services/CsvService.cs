...
/// <summary>
/// Unified CSV service using CsvHelper for maximum simplicity and reliability
/// </summary>
public class CsvService(INotificationService notificationService) : ICsvService
{
    ...
    public async Task WriteCsvFromDataTableAsync(DataTable dataTable, string filePath)
    {
        try
        {
            ...
            _notificationService.ShowSuccess($"CSV file written: {Path.GetFileName(filePath)}");
        }
        catch (Exception ex)
        {
            _notificationService.ShowError($"Error writing CSV file: {ex.Message}");
            throw;
        }
    }
    ...
    public async Task<CsvMetadata> GetMetadataAsync(string filePath)
    {
        try
        {
            ...
            _notificationService.ShowInfo($"CSV metadata analyzed: {metadata}");
            return metadata;
        }
        catch (Exception ex)
        {
            _notificationService.ShowError($"Error analyzing CSV metadata: {ex.Message}");
            return new CsvMetadata();
        }
    }
}